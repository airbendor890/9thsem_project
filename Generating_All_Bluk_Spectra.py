    ...
    for alpha in [1,2,3,4]:
        ...
        for n_1 in [-1,0,1]:
            for n_2 in [-1,0,1]:
                for n_3 in [-1,0,1]:
                    for beta in [1,2,3,4]:
                        sum1 += (S)*(D(J_Matrix, J_Matrix, alpha, -n_1, -n_2, -n_3, beta, 1, 3, x[alpha-1], x[alpha+3], x[beta-1], x[beta+3]))
                        sum2 += (S)*(D(J_Matrix, J_Matrix, alpha, -n_1, -n_2, -n_3, beta, 2, 3, x[alpha-1], x[alpha+3], x[beta-1], x[beta+3]))
        sum1 -= B(1, x[alpha-1], x[alpha+3], B_ext)/2
        sum2 -= B(2, x[alpha-1], x[alpha+3], B_ext)/2
        F.appenD(J_Matrix, sum1)
        F.appenD(J_Matrix, sum2)
    return F
...
def Eigen_Value_finder(J_Matrix, q, Theta_s, Phi_s):
...
def Draw_Band(Path, J_Matrix, J_int, D_int, K_int, Angles):
    ...
    for p in range(No_points-1):
        ...
        for i in range(Points_in_1_path):
            ...
            #print(q)
            EV = np.sort(abs(Eigen_Value_finder(J_Matrix, q, Theta_s, Phi_s)))
            x += [i+x_l]
            ...
            for j in range(8):
                y[j] += [EV[j]]
        x_l = x[-1]+1
    EV = np.sort(abs(Eigen_Value_finder(J_Matrix, locals()[Path[No_points-1]], Theta_s, Phi_s)))
    x += [x_l]
...
guess = np.zeros(8)
fo1 = open('Classical_spin_orientations_FM_D_p3.dat', "r")
fo2 = open('spin_orientations_fm_D_mp3_h_p1_100.dat', "r")
#fo = open('spin_orientations_afm_D_p30_h_p1_100.dat', "r")
#fo = open('spin_orientations_afm_D_mp3_h_p1_100_linear_terms_min.dat', "r")
ls_eq1 = fo1.read().split('\n')
fo1.close()

VV_Angles = np.zeros(8)
ls_eq2 = fo2.read().split('\n')
fo2.close()
for r in range(len(ls_eq1)):
    data = [float(j) for j in ls_eq1[r].split()]
    VV_data = [float(j) for j in ls_eq2[r].split()]
    K_int = data[0]
    for i in range(8):
        guess[i] = data[i+1]
        VV_Angles[i] = VV_data[i+1]
    J_Matrix = Interaction_Matrix(J_int, D_int, K_int)
    res = Energy_minimization(guess, B_ext, J_Matrix)
    Angles = res.x
    print(np.isclose(Classical_Energy_at(Angles, B_ext, J_Matrix), Classical_Energy_at(VV_Angles, B_ext, J_Matrix), atol=1e-3))
...
#Draw_Band(['G','X','W','G','L','W','U','X','K','G'], J_Matrix, J_int, D_int, K_int, Angles)